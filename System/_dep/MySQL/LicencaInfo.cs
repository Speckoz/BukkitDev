        ...
        private async Task<List<string>> PegarParam(string @ref, string dataPesquisar)
        {
            try
            {
                using (MySqlConnection con = new MySqlConnection(await PegarConexaoMySQL.ConexaoAsync()))
                {
                    ...
                    using (MySqlCommand get = new MySqlCommand($"select * from LicencaList where (LicencaKey = @a || ClienteID = @a || IPPermitido = @a) {(dataPesquisar != null ? "&& DataCriacao = @b" : string.Empty)}", con))
                    {
