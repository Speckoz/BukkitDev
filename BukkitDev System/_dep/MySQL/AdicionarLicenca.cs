...
namespace BukkitDev_System._dep.MySQL
{
	...
	{
		private const string CmdText = "insert into licencelist values (@a, @b, @c, @d, @e);";
		...
		private static void Parametros(uint user, uint pluginCod, string key, bool global, MySqlCommand add)
		{
			...
			_ = add.Parameters.Add(new MySqlParameter("@e", $"{System.DateTime.Now.ToString("yyyy-MM-dd")} as {System.DateTime.Now.ToString("HH:mm")}"));
		}
	}
}
